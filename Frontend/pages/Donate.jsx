...
const Donate = ({ user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const success = await (async () => {
      // If logged in, use existing authenticated flow
      if (user) {
      ...
      // Public (no login) donation submit
      try {
        const API_URL =
          import.meta.env.VITE_API_URL || "http://localhost:5000/api";
        const response = await axios.post(
          `${API_URL}/public-donation`,
          {
            data: formData,
        ...
        >
          <div className="mb-8">
            ...
            <p className="text-gray-500 text-sm mt-1">
              Please fill the details after making the payment. Login is not required.
            </p>
          </div>
