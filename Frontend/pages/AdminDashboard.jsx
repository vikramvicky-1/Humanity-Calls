...
const AdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState("add-gallery");
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = sessionStorage.getItem("adminToken");
      await axios.delete(`${import.meta.env.VITE_API_URL}/gallery/${imageToDelete}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div className="h-screen bg-bg flex flex-col overflow-hidden">
      {/* Admin Navbar */}
      <nav className="bg-white border-b border-border px-6 py-4 flex items-center justify-between z-50 shadow-sm shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-xl font-bold text-blood tracking-tight hidden sm:block">
              Humanity Calls
            </span>
          </div>
        </div>
        <button
          ...
          className="flex items-center gap-2 text-blood font-bold hover:bg-blood/5 px-4 py-2 rounded-lg transition-all text-sm uppercase tracking-wider"
        >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside
          className={`
          fixed lg:static inset-y-0 left-0 w-64 bg-white border-r border-border z-[100] lg:z-auto transform transition-transform duration-300 ease-in-out shrink-0
          ...
          <div className="p-6 md:p-10 lg:p-12 flex-grow">
            <div className="max-w-6xl mx-auto w-full">
              {activeTab === "add-gallery" ? (
                <div className="bg-white rounded-3xl shadow-xl border border-border p-8 md:p-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="bg-white rounded-3xl shadow-xl border border-border p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    ...
                    >
                      ...
                      {PROGRAMS.map((program) => (
                        <option key={program.id} value={program.id}>
                          {program.id.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")}
                        </option>
                      ))}
                      <option value="general">General Events</option>
                    </select>
                  ...
                  {isLoadingGallery ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                      {galleryImages.map((img) => (
                        <div key={img._id} className="group relative aspect-square rounded-2xl overflow-hidden border border-border bg-bg shadow-sm hover:shadow-md transition-all">
                          <img
                            src={img.imageUrl}
                            alt="Gallery"
                            className="w-full h-full object-cover"
                          ...
                          <div className="absolute top-2 right-2">
                            <button
                              ...
                              className="bg-blood text-white p-3 rounded-xl shadow-lg hover:scale-110 active:scale-95 transition-all z-10"
                              title="Delete Image"
                            >
                              <FaTrashAlt size={16} />
                            </button>
                    ...
                    <div className="py-24 text-center">
                      ...
                      <p className="text-text-body/60 italic">Upload some images to see them here.</p>
                    </div>
                  )}
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full relative z-10 shadow-2xl border border-border animate-in zoom-in-95 duration-300">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-primary mb-3">Confirm Delete</h3>
